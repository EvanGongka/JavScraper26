            ...
            for entry in entries:
                ...
                try:
                    ...
                    if not providers:
                        ...
                        continue
                    self.on_log(
                        f"[{entry.code}] 本次尝试站点顺序: {' -> '.join(provider.site_name for provider in providers)}"
                    )
                    ...
                    self.on_status(entry.code, "失败")
                    self.on_log(f"[{entry.code}] 处理失败，已继续处理其他条目: {exc}")
                    if self.on_error:
                        try:
                            self.on_error(entry.code, exc)
                        except Exception as callback_error:
                            # 诊断回调不能影响后续条目和最终清单生成。
                            self.on_log(f"[{entry.code}] 异常日志回调失败，已继续: {callback_error}")
        finally:
